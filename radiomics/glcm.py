...
def _calculateMatrix(self, voxelCoordinates=None, specified_angle=None):
    ...
    # Find the index of the specified angle
    angle_index = numpy.where((angles == specified_angle).all(axis=1))[0][0]

    self.logger.debug('Process calculated matrix for specified angle: %s', specified_angle)
    ...
    if self.weightingNorm is not None:
        ...
        weights = numpy.empty(1)
        if self.weightingNorm == 'infinity':
            weights[0] = numpy.exp(-max(numpy.abs(specified_angle) * pixelSpacing) ** 2)
        elif self.weightingNorm == 'euclidean':
            weights[0] = numpy.exp(-numpy.sum((numpy.abs(specified_angle) * pixelSpacing) ** 2))
        elif self.weightingNorm == 'manhattan':
            weights[0] = numpy.exp(-numpy.sum(numpy.abs(specified_angle) * pixelSpacing) ** 2)
        elif self.weightingNorm == 'no_weighting':
            weights[0] = 1
        else:
            self.logger.warning('weighting norm "%s" is unknown, W is set to 1', self.weightingNorm)
            weights[0] = 1

        P_glcm = P_glcm * weights[None, None, None, :]
